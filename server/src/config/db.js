...
// Only set custom DNS in local non-serverless dev if needed
...
const connectDB = async () => {
  ...
  connectionPromise = (async () => {
    ...
    try {
      ...
      if (error.message.includes('Could not connect to any servers') || error.message.includes('whitelist') || error.message.includes('buffering timed out')) {
        console.warn('\n-------------------------------------------------------------');
        console.warn('⚠️  MONGODB ATLAS IP WHITELIST NOTICE:');
        console.warn('   Please ensure MongoDB Atlas allows access from everywhere:');
        console.warn('   Go to https://cloud.mongodb.com -> Security -> Network Access -> Add IP Address: 0.0.0.0/0');
        console.warn('-------------------------------------------------------------\n');
      }
